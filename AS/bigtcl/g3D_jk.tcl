...
proc g_3D {mylist opis1 opis2 rka dra myinput myoutput} {
...
bigdcd g3D $file1 $file2 $file3 $file4 $file5
  }
5 {
puts "No 5"
set file1 [lindex $mylist 0]
...
puts "Nie podales wlasciwej liczby plikow"
    ...
    set norm [expr 4.0*$pi*$dr*$rho]
    set gnorm 0.0
    puts " rho: $rho , norma: $norm , licznik: $licznik"
 foreach xr $r xg $g {
    set rr2 2.0
    if { $xr >= $dr } {
    set rr2 [expr 2.0*$norm*$xr*$xr]
    }
    puts $out1 " $xr [expr 2.0*$xg/$rr2/$licznik] $xg $rr2"
                                      }
puts " !!!!!!!!!!!!!!!!!!!!!!!!!!!"
    close $out1
...
proc g3D { frame } {
 ...
 global dr
      ...
      set rho [expr $rho + $n1*$n2/($a0*$b0*$c0)]
      ...
      foreach x1 [$atom1 get x] y1 [$atom1 get y] z1 [$atom1 get z] {
           ...
           set r1 [expr sqrt($dx*$dx + $dy*$dy +$dz*$dz) ]
